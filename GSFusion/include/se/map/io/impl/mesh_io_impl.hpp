...
template<typename FaceT>
int save_mesh_vtk(const Mesh<FaceT>& mesh_M, const std::string& filename, const Eigen::Matrix4f& T_OM)
{
    ...
    if constexpr (FaceT::colour) {
        ...
        for (const auto& face : mesh_M) {
            for (const auto& colour : face.vertex_colours) {
                file << static_cast<float>(colour.r) / UINT8_MAX << " " << static_cast<float>(colour.g) / UINT8_MAX << " " << static_cast<float>(colour.b) / UINT8_MAX << "\n";
            }
        }
        file << "\n";
    }
    ...
    // Write the face class ID.
    if constexpr (FaceT::semantics) {
        file << "SCALARS class_id unsigned_char\n";
        file << "LOOKUP_TABLE default\n";
        for (const auto& face : mesh_M) {
            file << static_cast<int>(face.class_id) << "\n";
        }
    }
...
template<typename FaceT>
int save_mesh_ply(const Mesh<FaceT>& mesh_M, const std::string& filename, const Eigen::Matrix4f& T_OM)
{
    // Open the file for writing.
    std::ofstream file(filename.c_str());
    if (!file.is_open()) {
    ...
    for (const auto& face : mesh_M) {
        for (size_t v = 0; v < FaceT::num_vertexes; ++v) {
            ...
            if constexpr (FaceT::colour) {
                ...
                file << " " << static_cast<int>(colour.r) << " " << static_cast<int>(colour.g) << " " << static_cast<int>(colour.b);
            }
            file << "\n";
        }
    }
    ...
    for (size_t f = 0; f < num_faces; ++f) {
        ...
        const Eigen::Vector3i rgb = se::colours::scale[mesh_M[f].max_vertex_scale].template cast<int>();
        file << " " << rgb.x() << " " << rgb.y() << " " << rgb.z();
        // Write the face class ID.
        if constexpr (FaceT::semantics) {
            file << static_cast<int>(mesh_M[f].class_id) << "\n";
        }
        file << "\n";
    }
...
template<typename FaceT>
int save_mesh_obj(const Mesh<FaceT>& mesh_M, const std::string& filename, const Eigen::Matrix4f& T_OM)
{
    // Open the file for writing.
    std::ofstream file(filename.c_str());
    if (!file.is_open()) {
