...
```text
1. 从 hda_roi_semcom_dataset/scenes/<scene> 重建 manifest
2. 训练指定场景的通信模型
...
```bash
/home/king/miniconda3/envs/hda_roi_semcom/bin/python scripts/run_scene_pipeline.py \
  --scene-id fushimi \
  --dry-run
```
...
```text
/home/king/Downloads/Projects/TCOM/datasets/hda_roi_semcom_dataset/scenes/fushimi/observations/traj*/frame_manifest.jsonl
/home/king/Downloads/Projects/TCOM/datasets/hda_roi_semcom_dataset/scenes/fushimi/observations/traj*/poses/*.json
```
...
```bash
/home/king/miniconda3/envs/hda_roi_semcom/bin/python scripts/train.py \
...
```bash
/home/king/miniconda3/envs/hda_roi_semcom/bin/python scripts/build_offline_lookup_table.py \
  --config configs/default.yaml \
...
```bash
/home/king/miniconda3/envs/hda_roi_semcom/bin/python scripts/convert_magician_trajectories.py \
  --dataset-root /home/king/Downloads/Projects/TCOM/datasets/hda_roi_semcom_dataset \
...
`--input-dir` / `--inputs` 只用于外部轨迹 CSV；使用现有 `hda_roi_semcom_dataset` 时不需要它们。
...
src/models/hda_roi.py                   RGB JSCC + depth/mask 数字链路整体系统
