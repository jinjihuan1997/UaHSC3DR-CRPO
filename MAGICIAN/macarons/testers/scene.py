...
import pickle
import lmdb
from macarons.utility.magician_utils import generate_key_value_splited_dict, line_segment_mesh_intersection
...
def compute_trajectory(params, macarons, camera, gt_scene, surface_scene, 
    ...
    while pose_i <= params.n_poses_in_trajectory:
        ...
        timing_record['valid_neighbors'] = int(len(valid_neighbors))
    
        max_coverage_gain = -1.
        next_idx = valid_neighbors[0]

        stage_start = _profile_now(device)
        coverage_candidate_evaluations = 0
        for neighbor_i in range(len(valid_neighbors)):
            ...
            with torch.no_grad():
                coverage_candidate_evaluations += 1
        ...
        timing_record['nbv_decision_time_s'] = _profile_now(device) - stage_start
        timing_record['coverage_candidate_evaluations'] = int(coverage_candidate_evaluations)
    ...
    return coverage_evolution, camera.X_cam_history, camera.V_cam_history, full_pc, full_pc_colors, full_pc_idx, timing_records
    ...
    for i in range(len(dataloader.dataset)):
        ...
        for i_scene in range(batch_size):
            ...
            for start_cam_idx_i in range(len(settings.camera.start_positions)):
                ...
                coverage_evolution, X_cam_history, V_cam_history, full_pc, full_pc_colors, full_pc_idx, timing_records = compute_trajectory(params, macarons,
                                                                                      camera,
                ...
                # reconstructed_json_name = os.path.join(results_dir, "your" + scene_name + '.json')
