...
def load_pretrained_macarons(pretrained_model_path, device, learn_pose=False, ddp=False):
    ...
    weights_path = os.path.join(weights_dir_path, pretrained_model_path)
    nested_weights_path = os.path.join(weights_dir_path, "macarons", pretrained_model_path)
    if not os.path.exists(weights_path) and os.path.exists(nested_weights_path):
        weights_path = nested_weights_path
    macarons.load_state_dict(torch.load(weights_path, map_location=device), ddp=ddp)
